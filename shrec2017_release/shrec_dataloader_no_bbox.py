...
class SHRECLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_data = np.load(
            insert(self.prefix.format(splitLine[0], splitLine[1], splitLine[2], splitLine[3]), "Background", 11)
            + "/pts_label.npy")[:, :, :7]
        ...
        if self.phase == "train":
            input_data = input_data[:, :, torch.randperm(input_data.shape[2])[:self.pts_size]]
        else:
            input_data = input_data[:, :, ::input_data.shape[2] // self.pts_size]
        # B * (4 + others) * 32 * 128
